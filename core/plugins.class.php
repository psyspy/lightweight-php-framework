...
class plugins {
	 ...
	 */
	public static function start($folder='plugins/') {
		...
		if(file_exists($folder))
		{
			...
			foreach($pluginFiles as $name)
			{
				...
				if(is_dir($folder.$name) and $name!='.' and $name!='..')
				{
					// call the plugin system and load it
					$plugin_file = $folder.$name.'/'.$name.'.class.php';

					if(file_exists($plugin_file))
					{
					
						self::load($plugin_file, $name);
					
					}
					else
					{
						error::add($name,'Plugin could not be loaded');
					}
            		...
            		foreach(self::$plugins[$hook] as $name)
            		{
            			...
            			if(!is_array($params))
                		{
                			// call plugin without params
                    			call_user_func(array(new $name, $hook));
                		} else {
                			// call plugin with params
                    			call_user_func_array(array(new $name, $hook), $params);
            			}
            		}
        	}
	}
